...
async def load_links(tg_chan: list, tg_cat: dict):
...
async def delete_category_full(category, tg_channels: list):
...
async def delete_single_link(link: str, tg_channels: list):
    async with aiosqlite.connect(DB_FILE) as db:
        db.row_factory = aiosqlite.Row
