...
async def load_links(vk: VKRealTimeManager):
...
async def delete_category_full(category, vk_manager):
...
async def delete_single_link(link: str, vk_manager):
    async with aiosqlite.connect(DB_FILE) as db:
        db.row_factory = aiosqlite.Row
        ...
        async with db.execute(
            ...
            link_data = cursor.fetchone() # Избегаем пересечения имени переменной link с результатом
            link_row = await link_data
        
        if link_row:
            link_url = link_row["url"]
            
            # 2. Удаляем из оперативной памяти
            if link_row["soc_media"] == "vk":
