...
class ConnectionManager:
    ...
    async def broadcast_to_category(self, message: dict, category: str):
        ...
        if category not in self.active_connections:
            return  # Если эту категорию сейчас никто не смотрит, просто игнорируем пуш
...
async def websocket_endpoint(websocket: WebSocket):
    # Считываем категорию, которую нам прислал JavaScript Максима
    ...
    try:
        while True:
            ...
            # Нам не нужно принимать данные от клиента (receive), мы только пушим (send)
...
async def delete_category(category,vkman=Depends(get_vkman)):
    await delete_category_full(category=category,tg_channels=tg_channels,vk_manager=vkman)
